...
public record class Synchronizer(IAdo Ado, IGitHub GitHub) : ISynchronizer
{
    ...
    public async Task<WorkItem> UpdateState(WorkItem workItem, Issue issue)
    {
        try
        {
            ...
            {
                AnsiConsole.MarkupLine($"[bold yellow]Status of work item {workItem.ReadableLink()} not updated, as GitHub issue {issue.HtmlUrl} may be missing a triage label.[/]. Work item state: {issue.WorkItemState()}.");
                return workItem;
            }
        }
